...
## 更新成就进度
func _update_achievement_progress(achievement_id: String, progress: int) -> void:
	"""更新成就进度"""
	if not achievements.has(achievement_id):
		return
...
## 获取排行榜分数
func get_leaderboard_scores(_leaderboard_name: String, _start: int = 0, _count: int = 10) -> Array:
	"""获取排行榜分数"""
	if _leaderboard_name.is_empty() or _start < 0 or _count <= 0:
		return []
...
## 打开Steam商店页面
func _open_store_page(app_id: int = 0) -> void:
	"""打开Steam商店页面"""
	if not is_steam_available:
		return
...
## 打开Steam社区页面
func _open_community_page(app_id: int = 0) -> void:
	"""打开Steam社区页面"""
	if not is_steam_available:
		return
...
## 检查Steam Overlay是否可用
func _is_overlay_available() -> bool:
	"""检查Steam Overlay是否可用"""
	if not is_steam_available:
		return false
...
## 激活Steam Overlay
func _activate_overlay() -> void:
	"""激活Steam Overlay"""
	if not _is_overlay_available():
		return
...
func _open_overlay_url(url: String) -> void:
...
## 上传统计数据
func _upload_statistic(stat_name: String, value: float) -> void:
	"""上传统计数据到Steam"""
	if not is_steam_available:
		return
...
func on_game_ended(success: bool, result: Dictionary) -> void:
	...
	if is_steam_available:
		...
		if result.has("distance"):
			_upload_statistic("total_distance", result["distance"])
		if result.has("elevation_gain"):
			_upload_statistic("total_elevation", result["elevation_gain"])
...
func _check_achievements(result: Dictionary) -> void:
	...
	if SaveManager.get_player_data().get("total_games_won", 0) == 0:
		_unlock_achievement("first_step")

	# 距离成就
	var total_distance = SaveManager.get_player_data().get("hiking_points", 0) / 2000.0
	_update_achievement_progress("distance_10km", int(total_distance))
	_update_achievement_progress("distance_100km", int(total_distance))

	# 爬升成就
	var total_elevation = SaveManager.get_player_data().get("elevation_gain", 0)
	_update_achievement_progress("elevation_1000m", int(total_elevation))

	# 连击成就
	var max_combo = result.get("max_combo", 0)
	_update_achievement_progress("combo_10", max_combo)

	# 环保成就
	var env_value = result.get("environmental_value", 0)
	_update_achievement_progress("environmentalist", env_value)
